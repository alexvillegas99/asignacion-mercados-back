...
export class SolicitudesService {
 ...
 async postular(solicitudId: string, ip: string) {
    ...
    try {
      sessionId = await this.subirPdfYObtenerRespuesta(); // <- tu método que retorna string
      this.logger.debug(`[POSTULAR] SESSION_ID obtenido=${sessionId || ''}`);
      if (!sessionId) {
        throw new BadRequestException('SESSION_ID es requerido para saveSolicitud');
      }
    } catch (e) {
      this.logger.error(`[POSTULAR] Error obteniendo SESSION_ID: ${e instanceof Error ? e.message : e}`);
      throw e;
    }
    ...
    this.logger.debug(`[POSTULAR] Solicitud cargada estado=${solicitud.estado} stall=${solicitud.stall}`);
    ...
    this.logger.debug(`[POSTULAR] Guard yaTienePuesto=${!!yaTienePuesto} cedula=${cedulaMask}`);

    if (yaTienePuesto) {
      this.logger.warn(`[POSTULAR] Usuario ya tiene puesto asignado/reservado cedula=${cedulaMask}`);
      throw new BadRequestException('Ya tienes un puesto asignado o reservado.');
    }
    ...
    this.logger.debug(`[POSTULAR] Guard ordenActiva=${!!ordenActiva} cedula=${cedulaMask}`);

    if (ordenActiva) {
      this.logger.warn(`[POSTULAR] Usuario tiene orden activa en proceso cedula=${cedulaMask}`);
    ...
    this.logger.debug(`[POSTULAR] Guard otraSolicitud=${!!otraSolicitud} cedula=${cedulaMask}`);

    if (otraSolicitud) {
      this.logger.warn(`[POSTULAR] Ya existe otra solicitud en curso cedula=${cedulaMask}`);
    ...
    this.logger.debug(`[POSTULAR] Intentando reservar stall=${solicitud.stall} hasta=${aprobarAntesDe.toISOString()}`);
    ...
    if (!stall) {
      this.logger.warn(`[POSTULAR] Conflicto: el puesto ya no está libre stall=${solicitud.stall}`);
      throw new BadRequestException('Conflicto: el puesto ya no está libre');
    }
    this.logger.log(`[POSTULAR] Puesto reservado stall=${stall._id} estado=${stall.estado}`);

    // 2) Crear Orden interna
    const ultimo: any = await this.ordenModel.findOne({}, {}, { sort: { secuencial: -1 } }).lean();
    ...
    this.logger.log(`[POSTULAR] Orden creada id=${orden._id} secuencial=${secuencial} ref=${referencia}`);

    await this.solicitudModel.findByIdAndUpdate(solicitud._id, {
      $set: { estado: 'POSTULADA', ordenId: orden._id },
    });
    this.logger.debug(`[POSTULAR] Solicitud actualizada a POSTULADA solicitud=${solicitud._id} orden=${orden._id}`);

    // 3) LLAMADA NUEVA: saveSolicitud (externo)
    ...
    const codPuesto = this.extractCodPuestoFromCode(stall.code); // e.g. "2942"
    ...
    // Log seguro del payload (enmascarado)
    const payloadLog = {
      ...payloadExterno,
      SESSION_ID:payloadExterno.SESSION_ID,
      expositor: {
        ...payloadExterno.expositor,
        id: payloadExterno.expositor.id,
        telefono: payloadExterno.expositor.telefono,
        correo:payloadExterno.expositor.correo,
      },
    };
    this.logger.debug(`[POSTULAR] Payload externo (mask): ${this.s(payloadLog)}`);

    let respuestaExterna: any = null;
    const url = 'https://appbackend.ambato.gob.ec:3002/api/WsFinados/rest/metodo/saveSolicitud';
    ...
    try {
      ...
      this.logger.log(`[POSTULAR] ← saveSolicitud status=${resp?.status ?? 'n/a'} dt=${dt}ms`);
      this.logger.debug(`[POSTULAR] Respuesta externa (trim): ${this.s(respuestaExterna)}`);

      // Guarda respuesta externa en la orden
      await this.ordenModel.findByIdAndUpdate(orden._id, { $set: { pagoExterno:respuestaExterna } });
      this.logger.debug(`[POSTULAR] Orden ${orden._id} almacenó respuesta externa`);
    } catch (e: any) {
      const dt = Date.now() - tHttp;
      const status = e?.response?.status;
      const data = e?.response?.data;
      this.logger.error(`[POSTULAR] saveSolicitud externo ERROR status=${status ?? 'n/a'} dt=${dt}ms body=${this.s(data)}`);

      // Compensación
      try {
        await this.stallModel.findByIdAndUpdate(stall._id, { $set: { estado: 'LIBRE', reservadoHasta: null } });
        this.logger.warn(`[POSTULAR][compensación] Stall ${stall._id} devuelto a LIBRE`);
      } catch (e2) {
        this.logger.error(`[POSTULAR][compensación] Error liberando stall: ${e2 instanceof Error ? e2.message : e2}`);
      }
      ...
      try {
        ...
        this.logger.warn(`[POSTULAR][compensación] Solicitud ${solicitud._id} revertida a EN_SOLICITUD`);
      } catch (e3) {
        this.logger.error(`[POSTULAR][compensación] Error revirtiendo solicitud: ${e3 instanceof Error ? e3.message : e3}`);
      }

      try {
        await this.ordenModel.findByIdAndUpdate(orden._id, {
          $set: { estado: 'RECHAZADA', observacion: 'Error en saveSolicitud externo' },
        });
        this.logger.warn(`[POSTULAR][compensación] Orden ${orden._id} marcada RECHAZADA`);
      } catch (e4) {
        this.logger.error(`[POSTULAR][compensación] Error marcando orden rechazada: ${e4 instanceof Error ? e4.message : e4}`);
      }

      throw new BadRequestException('No se pudo crear la solicitud externa (saveSolicitud).');
    }

    // Devuelve la orden con la respuesta externa adjunta
    const dtAll = Date.now() - t0;
    this.logger.log(`[POSTULAR] OK solicitudId=${solicitudId} orden=${orden._id} tiempoTotal=${dtAll}ms`);
    orden = {
  ...
  try {
    ...
    this.logger.error(`[DEUDAS] Error consultando ${url}: ${e?.message || e}`);
    return [];
  }
}
...
async sincronizarDeudas() {
  ...
  for (const item of lista) {
    try {
      // Solo procesar RECAUDADO
      if (item.EST_FACTURA !== 'RECAUDADO') continue;

      const cedulaApi = String(item.ID_EXPOSITOR).trim();
      const codPuesto = String(item.COD_PUESTO).trim();

      // 🔎 Buscar stall por code
      const stall = await this.stallModel.findOne({
        code: { $regex: `T-${codPuesto}$`, $options: 'i' },
      }).lean();

      if (!stall) {
        this.logger.warn(`[DEUDAS] No se encontró stall para COD_PUESTO=${codPuesto}`);
        continue;
      }
      ...
      if (!orden) {
        this.logger.debug(`[DEUDAS] No hay orden activa para cedula=${cedulaApi}, puesto=${codPuesto}`);
        continue;
      }

      // 🔹 Adjudicar (llama a aprobar)
      this.logger.log(`[DEUDAS] Adjudicando orden=${orden._id} puesto=${codPuesto} cedula=${cedulaApi}`);
