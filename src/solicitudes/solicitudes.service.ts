...
export class SolicitudesService {
  ...
  async postular(solicitudId: string, ip: string) {
    ...
    this.logger.log(
      `[POSTULAR] Orden creada id=${orden._id} secuencial=${secuencial} ref=${referencia}`,
    );
    ...
    this.logger.debug(
      `[POSTULAR] Payload externo (mask): ${this.s(payloadLog)}`,
    );
    ...
    try {
      ...
      this.logger.debug(
        `[POSTULAR] Respuesta externa (trim): ${this.s(respuestaExterna)}`,
      );
      ...
      try {
        ...
        this.logger.warn(
          `[POSTULAR][compensación] Solicitud ${solicitud._id} revertida a EN_SOLICITUD`,
        );
      } catch (e3) {
        this.logger.error(
    ...
    this.logger.log(
      `[POSTULAR] OK solicitudId=${solicitudId} orden=${orden._id} tiempoTotal=${dtAll}ms`,
    );
    orden = {
