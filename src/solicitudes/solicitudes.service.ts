...
async crearSolicitud(dto: any) {
  try {
    ...
    const usuario = typeof dto.usuario === 'string' ? dto.usuario.trim() : null;
 ...
 async postular(solicitudId: string, ip: string) {
  ...
  // 🔹 Modo manual
  const isManual =
    Boolean((solicitud as any)?.manual) || Boolean((solicitud as any)?.usuario);
  ...
  await this.solicitudModel.findByIdAndUpdate(solicitud._id, {
    $set: { estado: 'POSTULADA', ordenId: orden._id, manual: isManual, usuario: usuarioSolicitante },
  });
  this.logger.debug(
  ...
  if (!isManual) {
    ...
    try {
      ...
      await this.ordenModel.findByIdAndUpdate(orden._id, {
        $set: { estado: 'RECHAZADA', observacion: 'Error al obtener SESSION_ID' },
      });
      throw e;
    }
